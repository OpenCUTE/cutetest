...
inline vfloat32m2_t vec_sin_small(vfloat32m2_t x, size_t vl) {
    // 计算 sin(x) 的泰勒展开，适用于小角度 x
    // sin(x) ≈ x - x³/3! + x⁵/5! - x⁷/7! + x⁹/9! - x¹¹/11!
    // sin(x) ≈ x * (c₁ + x² * (c₃ + x² * (c₅ + x² * (c₇ + x² * (c₉ + x² * c₁₁)))))
    vfloat32m2_t x2 = __riscv_vfmul_vv_f32m2(x, x, vl); // x²
    vfloat32m2_t c1  = __riscv_vfmv_v_f_f32m2(1.0f, vl);
    vfloat32m2_t c3  = __riscv_vfmv_v_f_f32m2(-0.166666666667f, vl);
    vfloat32m2_t c5  = __riscv_vfmv_v_f_f32m2(0.008333333333f, vl);
    vfloat32m2_t c7  = __riscv_vfmv_v_f_f32m2(-0.0001984126984f, vl);
    vfloat32m2_t c9  = __riscv_vfmv_v_f_f32m2(0.000002755731922f, vl);
    vfloat32m2_t c11 = __riscv_vfmv_v_f_f32m2(-0.000000025052108f, vl);
    vfloat32m2_t result;
    // 计算并累加每一项
    // res = a1 + a2 * a3
    result = __riscv_vfmacc_vv_f32m2(c9,     c11, x2, vl);
    result = __riscv_vfmacc_vv_f32m2(c7,  result, x2, vl);
    result = __riscv_vfmacc_vv_f32m2(c5,  result, x2, vl);
    result = __riscv_vfmacc_vv_f32m2(c3,  result, x2, vl);
    result = __riscv_vfmacc_vv_f32m2(c1,  result, x2, vl);
    result = __riscv_vfmul_vv_f32m2(result, x, vl); // 最后乘以 x
    return result;
}


inline vfloat32m2_t vec_sin(vfloat32m2_t x, size_t vl) {
    const float PI = 3.14159265359f;
    const float ONE_DIV_PI = 1.0f / PI;
    const float PI_DIV_2 = PI / 2.0f;
    const float MAGIC = 0x1.8p23f; // 2²³ + 2²²，用于取整
    // vfloat32m2_t new_rad = __riscv_vfadd_vv_f32m2(x, __riscv_vfmv_v_f_f32m2(PI_DIV_2, vl), vl);
    vfloat32m2_t new_rad = __riscv_vfadd_vf_f32m2(x, PI_DIV_2, vl);
    // vfloat32m2_t pi_vec = __riscv_vfmv_v_f_f32m2(PI, vl);
    // vfloat32m2_t round = __riscv_vfdiv_vv_f32m2(new_rad, pi_vec, vl);
    vfloat32m2_t round = __riscv_vfmul_vf_f32m2(new_rad, ONE_DIV_PI, vl);
    // vfloat32m2_t magic = __riscv_vfmv_v_f_f32m2(0x1.8p23f, vl); // 2²³ + 2²²，用于取整
    // round = __riscv_vfadd_vv_f32m2(round, magic, vl);
    round = __riscv_vfadd_vf_f32m2(round, MAGIC, vl);
    // round = __riscv_vfsub_vv_f32m2(round, magic, vl);
    round = __riscv_vfsub_vf_f32m2(round, MAGIC, vl);
    // new_rad = __riscv_vfsub_vv_f32m2(new_rad, __riscv_vfmul_vv_f32m2(round, pi_vec, vl), vl);
    new_rad = __riscv_vfsub_vv_f32m2(new_rad, __riscv_vfmul_vf_f32m2(round, PI, vl), vl);
    // new_rad = __riscv_vfsub_vv_f32m2(new_rad, __riscv_vfmv_v_f_f32m2(PI_DIV_2, vl), vl);
    new_rad = __riscv_vfsub_vf_f32m2(new_rad, PI_DIV_2, vl);
    // 计算 sin(new_rad)
    vfloat32m2_t sin_result = vec_sin_small(new_rad, vl);
    // 根据 round 的奇偶性调整符号
    vuint32m2_t round_int = __riscv_vfcvt_xu_f_v_u32m2(round, vl);
    vuint32m2_t round_odd_int = __riscv_vand_vx_u32m2(round_int, 1, vl);
    round_odd_int = __riscv_vsll_vx_u32m2(round_odd_int, 31, vl); // 将最低位移到符号位
    vfloat32m2_t sign = __riscv_vreinterpret_v_u32m2_f32m2(round_odd_int);
    sin_result = __riscv_vfsgnjn_vv_f32m2(sin_result, sign, vl);
    return sin_result;
}

inline vfloat32m2_t vec_cos(vfloat32m2_t x, size_t vl) {
    // 将 x 映射到 [-π, π]
    const float PI = 3.14159265359f;
    const float PI_DIV_2 = PI / 2.0f;
    // vfloat32m2_t new_rad = __riscv_vfadd_vv_f32m2(x, __riscv_vfmv_v_f_f32m2(PI_DIV_2, vl), vl);
...
void fuse_ops_DEQUANT_ROPE_BF16CVRT(void * input,void *output,void * input_scale,void *weight_scale,int dim_i,int dim_j,uint64_t input_stride,uint64_t output_stride, void* pos)
{
    ...
    for (int j = 0; j < seq_len; j++) {
        ...
        for (int k = 0, avl = half_dim, vl = 0; avl > 0; k += vl, avl -= vl) {
            ...
            // 加载rope_theta参数
            vfloat32m2_t theta_vec = __riscv_vle32_v_f32m2(rope_theta + k, vl);

            // 计算角度：angle = pos_ * inv_freq
            vfloat32m2_t angle_vec = __riscv_vfmul_vf_f32m2(theta_vec, pos_, vl);

            // 计算sin和cos
            vfloat32m2_t sin_vec, cos_vec;
            sin_vec = vec_sin(angle_vec, vl);
            cos_vec = vec_cos(angle_vec, vl);

            // 加载输入数据（实部和虚部）
            vint32m2_t real_in = __riscv_vlse32_v_i32m2(input_row + 2 * k, 2*sizeof(float), vl);//虽然慢，但是能够接受
            vint32m2_t imag_in = __riscv_vlse32_v_i32m2(input_row + 2 * k + 1, 2*sizeof(float), vl);//虽然慢，但是能够接受

            // 将I32转换为F32并应用缩放
            vfloat32m2_t real_in_f32 = __riscv_vfmul_vf_f32m2(__riscv_vfcvt_f_x_v_f32m2(real_in, vl), scale, vl);
            vfloat32m2_t imag_in_f32 = __riscv_vfmul_vf_f32m2(__riscv_vfcvt_f_x_v_f32m2(imag_in, vl), scale, vl);

            // 计算旋转：real_out = real*cos - imag*sin
            // res = a1 * a2 - a3
            vfloat32m2_t real_out = __riscv_vfmsub_vv_f32m2(real_in_f32, cos_vec,__riscv_vfmul_vv_f32m2(imag_in_f32, sin_vec, vl), vl);

            // 计算旋转：imag_out = real*sin + imag*cos
            // res = a2 * a3 + a1
            vfloat32m2_t imag_out = __riscv_vfmacc_vv_f32m2(__riscv_vfmul_vv_f32m2(real_in_f32, sin_vec, vl),imag_in_f32, cos_vec, vl);

            // 存储结果

            // 转化为fp16再存储
            vfloat16m1_t real_out_fp16 = __riscv_vfncvt_f_f_w_f16m1(real_out, vl);
            vfloat16m1_t imag_out_fp16 = __riscv_vfncvt_f_f_w_f16m1(imag_out, vl);
            __riscv_vse16_v_f16m1(output_row + k, real_out_fp16, vl);
            __riscv_vse16_v_f16m1(output_row + half_dim + k, imag_out_fp16, vl);
        }
    }
}
...
char fake_tcm[2 * 1024 * 1024] __attribute__((aligned(256)));

void * CUTE_result[4] = {(void *) (fake_tcm), (void *) (fake_tcm + SEQ_LEN * SEQ_LEN * 4), (void *) (fake_tcm + SEQ_LEN * SEQ_LEN * 4 * 2), (void *) (fake_tcm + SEQ_LEN * SEQ_LEN * 4 * 3)};//double buffer use shuttle tcm
// void * CUTE_result_layerwise[2] = {(void *) (fake_tcm), (void *) (fake_tcm + Tensor_M * EMBEDING_DIMENSION * 4)};//double buffer use shuttle tcm
void * TCM_BUFF = (void *) (fake_tcm);//2MB
