...
def run_command(x, progress, total, lock):
    ...
    for ddrtype in DDR_type:
        ...
        (set -o pipefail &&  ./build/simulator-chipyard.harness-CUTE4TopsShuttle512D512V512M512Sysbus512Membus1CoreConfig\
            +permissive \
            +dramsim +dramsim_ini_dir=../../dramsim_config/dramsim2_ini_{ddrtype}_per_s +max-cycles=800000000 +loadmem=../llama3_1B_{x}.riscv     \
            +verbose \
            +permissive-off \
            ../llama3_1B_{x}.riscv \
            \
            </dev/null  | tee ./log/llama3_1B_{x}_{ddrtype}_task.log)
        """

        script_path = f"./test/llama3_1B_{x}_{ddrtype}_task.sh"
