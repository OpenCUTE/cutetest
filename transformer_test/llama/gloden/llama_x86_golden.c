...
void rope(float *input, int pos, int dim_i, int dim_j, float *output, int stride_c) {
    ...
    for (int i = 0; i < dim_i; i++) {
        for (int j = 0; j < dim_j/2; j++) {
            ...
            // printf("input[%d][%d]: %f, input[%d][%d]: %f, output[%d][%d]: %f, output[%d][%d]: %f\n",
                //    i, j * 2, input[i * dim_j + j * 2],
                //    i, j * 2 + 1, input[i * dim_j + j * 2 + 1],
                //    i, j, output[i * stride_c + j],
                //    i, j + dim_j/2, output[i * stride_c + j + dim_j/2]);
            // exit(0);
        }
    }
}
