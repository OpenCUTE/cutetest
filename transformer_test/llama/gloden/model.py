...
def c_array(array):
    return str(array).replace("[", "{").replace("]", "}").replace("(", "").replace(")", "").replace("tensor", "")
...
class Model:
    ...
    def forward(self, inputs: torch.Tensor, use_kvcache: bool = True):
        ...
        f.write(f"#define INPUT_SIZE {seq_len}\n")
        f.write(f"#define RMS_EPSILON {self.rms_epsilon}\n")
        f.write(f"#define EMBEDING_DIMENSION {self.d}\n")
        f.write(f"#define KEY_DIMENSION {self.d_k}\n")
        f.write(f"#define VALUE_DIMENSION {self.d_v}\n")
        f.write(f"#define N_HEAD_Q {self.n_head_q}\n")
        f.write(f"#define N_HEAD_KV {self.n_head_kv}\n")
        f.write(f"#define MAX_CTX_LEN {self.max_ctx_len}\n")
        f.write(f"#define FFN_DIMENSION {self.d_ffn}\n")
        f.write("\n")
        f.write(f"static float rope_theta[KEY_DIMENSION/2] __attribute__((aligned(256))) = {c_array(self.rope_theta)};\n")
        ...
        for i in range(self.n_block):
            ...
            # print(identity)
            f.write(f"static float identity[INPUT_SIZE][EMBEDING_DIMENSION] __attribute__((aligned(256))) = {c_array(identity)};\n")
            hidden_states = self.rmsnorm(hidden_states, self.attn_norm_weight[i])                     # [sl, d]
            f.write(f"static float attn_norm_weight[EMBEDING_DIMENSION] __attribute__((aligned(256))) = {c_array(self.attn_norm_weight[i])};\n")
            # print(hidden_states)
            print(f"Hidden States: {c_array(hidden_states)}\n")
            # Attention
            # q: [nhq, sl, dk] = RoPE([sl, d] @ [nhq, dk, d])
            q = self.rope(hidden_states @ self.attn_q_weight[i].transpose(1, 2), pos=self.ctx_len)    # [nhq, sl, dk]
            f.write(f"static float attn_q_weight[N_HEAD_Q][KEY_DIMENSION][EMBEDING_DIMENSION] __attribute__((aligned(256))) = {c_array(self.attn_q_weight[i])};\n")
            # print(q)
            print(f"hidden_states @ self.attn_q_weight[i].transpose(1, 2): {hidden_states @ self.attn_q_weight[i].transpose(1, 2)}\n")
            print(f"Q: {c_array(q)}\n")
            f.write(f"static float attn_k_weight[N_HEAD_KV][KEY_DIMENSION][EMBEDING_DIMENSION] __attribute__((aligned(256))) = {c_array(self.attn_k_weight[i])};\n")
            f.write(f"static float attn_v_weight[N_HEAD_KV][VALUE_DIMENSION][EMBEDING_DIMENSION] __attribute__((aligned(256))) = {c_array(self.attn_v_weight[i])};\n")
            f.write(f"static float attn_o_weight[EMBEDING_DIMENSION][N_HEAD_Q * VALUE_DIMENSION] __attribute__((aligned(256))) = {c_array(self.attn_o_weight[i])};\n")
            f.write(f"static float ffn_norm_weight[EMBEDING_DIMENSION] __attribute__((aligned(256))) = {c_array(self.ffn_norm_weight[i])};\n")
            f.write(f"static float ffn_gate_weight[FFN_DIMENSION][EMBEDING_DIMENSION] __attribute__((aligned(256))) = {c_array(self.ffn_gate_weight[i])};\n")
            f.write(f"static float ffn_up_weight[FFN_DIMENSION][EMBEDING_DIMENSION] __attribute__((aligned(256))) = {c_array(self.ffn_up_weight[i])};\n")
            f.write(f"static float ffn_down_weight[EMBEDING_DIMENSION][FFN_DIMENSION] __attribute__((aligned(256))) = {c_array(self.ffn_down_weight[i])};\n")
    ...
    def rope(self, x: torch.Tensor, pos: int = 0):
        ...
        # print(f"idx: {c_array(idx)}")
        idx_theta = torch.einsum('s,d->sd', idx, self.rope_theta)         # [sl, d_rope // 2]
        # print(f"self.rope_theta: {c_array(self.rope_theta)}")
...
if __name__ == "__main__":
    ...
    login(token="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")
    # login(token="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")

    model_name = "meta-llama/Llama-3.2-1B-Instruct"
    gguf_path = "../models/Llama-3.2-1B-Instruct-Q8_0.gguf"

    client_fp32 = Client(model_name, gguf_path, max_ctx_len=8192, dtype="float32", device="cpu")
    client_fp16 = Client(model_name, gguf_path, max_ctx_len=8192, dtype="float16", device="cpu")
