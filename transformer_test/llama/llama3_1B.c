...
void fuse_ops_DEQUANT_ROPE_BF16CVRT(void * input,void *output,void * input_scale,void *weight_scale,int dim_i,int dim_j,uint64_t input_stride,uint64_t output_stride)
{
        ...
        for (int i = 0; i < n_head; i++) {
            for (int j = 0; j < seq_len; j++) {
                ...
                for (int k = 0, avl = half_dim, vl = 0; avl > 0; k += vl, avl -= vl) {
                    ...
                    // 转化为fp16再存储
                    vfloat16m4_t real_out_fp16 = __riscv_vfncvt_rod_f_f_w_f16m4(real_out, vl);
                    vfloat16m4_t imag_out_fp16 = __riscv_vfncvt_rod_f_f_w_f16m4(imag_out, vl);
                    __riscv_vse16_v_f16m4(output_f16 + output_offset + k, real_out_fp16, vl);
                    __riscv_vse16_v_f16m4(output_f16 + output_offset + half_dim + k, imag_out_fp16, vl);
                }
            }
        }
    }
...
void fuse_ops_DEQUANT_BF16CVRT(void * input,void *output,void * input_scale,void *weight_scale,int dim_i,int dim_j,uint64_t input_stride,uint64_t output_stride)
{
    //stage 1: dequant input(I32) with scale to f32
        ...
        for (int i = 0; i < n_head; i++) {
            for (int j = 0; j < seq_len; j++) {
                ...
                for (int k = 0, avl = headdim, vl = 0; avl > 0; k += vl, avl -= vl) {
                    ...
                    vfloat16m4_t deq_done_fp16 = __riscv_vfncvt_rod_f_f_w_f16m4(deq_done, vl);
                    __riscv_vse16_v_f16m4(output_f16 + output_offset + k, deq_done_fp16, vl);
                }
            }
        }
    }
...
//   void afater_operation(void * input,void *output,void * input_scale,void *weight_scale,int dim_i,int dim_j,uint64_t input_stride,uint64_t output_stride)
  void (*afater_operation)(void* ,void* ,void* ,void* ,int ,int ,uint64_t ,uint64_t ) = NULL;
