        ...
        for (int i = 0; i < n_head; i++) {
            for (int j = 0; j < seq_len; j++) {
                ...
                for (int k = 0, avl = half_dim, vl = 0; avl > 0; k += vl, avl -= vl) {
                    ...
                    // 加载输入数据（实部和虚部）
                    vint32m4_t real_in = __riscv_vlse32_v_i32m4(input_i32 + input_offset + 2 * k, 2*sizeof(float), vl);//虽然慢，但是能够接受
                    vint32m4_t imag_in = __riscv_vlse32_v_i32m4(input_i32 + input_offset + 2 * k + 1, 2*sizeof(float), vl);//虽然慢，但是能够接受
                    ...
                    __riscv_vse16_v_f16m2(output_f16 + output_offset + k, real_out_fp16, vl);
                    __riscv_vse16_v_f16m2(output_f16 + output_offset + half_dim + k, imag_out_fp16, vl);
                }
            }
        }
    }
        ...
        for (int i = 0; i < n_head; i++) {
            for (int j = 0; j < seq_len; j++) {
                ...
                for (int k = 0, avl = headdim, vl = 0; avl > 0; k += vl, avl -= vl) {
                    ...
                    vint32m4_t input_vec = __riscv_vle32_v_i32m4(input_i32 + input_offset + k, vl);
                    ...
                    __riscv_vse16_v_f16m2(output_f16 + output_offset + k, deq_done_fp16, vl);
                }
            }
        }
    }
...
inline void softmax_cvrtfp16(void* x, void* y, void* bitmask_ptr, int M, int N) {
    // printf("RISC-V specific implementation WORK IN PROGRESS\n");
    for (int i = 0; i < M; i++) {
        float* row_x = &(((float_t* )x)[i * N]);
        float* row_y = &(((float16_t* )y)[i * N]);
        size_t avl, vl;
        ...
        for (int j = 0, avl = N; avl > 0; j += vl, avl -= vl) {
            ...
            __riscv_vse16_v_f16m2((float16_t*)&row_y[j], normalized_fp16, vl);
        }
    }
...
void fuse_ops_MASKED_SOFTMAX_KVSCALE_BF16CVRT(void * input,void *output,void * input_scale,void *weight_scale,int dim_i,int dim_j,uint64_t input_stride,uint64_t output_stride, void* output_scale)
{
    ...
    for (int j = 0; j < seq_len; j++) {
        ...
        int input_offset = j * head_dim;
        int output_offset = j * head_dim;
        vl = __riscv_vsetvl_e32m4(avl);
        for (int k = 0, avl = seq_len, vl = 0; avl > 0; k += vl, avl -= vl) {
            ...
            __riscv_vse32_v_f32m4(softmabuf_f32 + output_offset + k, deq_done, vl);
        }
    }
    softmax_cvrtfp16(softmabuf_f32, output_f16, bitmask_ptr, seq_len, seq_len);
        ...
        for (int i = 0; i < n_head; i++) {
            for (int j = 0; j < seq_len; j++) {
                ...
                for (int k = 0, avl = headdim, vl = 0; avl > 0; k += vl, avl -= vl) {
                    ...
                    vint32m4_t input_vec = __riscv_vle32_v_i32m4(input_i32 + input_offset + k, vl);
                    vfloat32m4_t res_vec = __riscv_vle32_v_f32m4(output_f32 + output_offset + k, vl);

                    vfloat32m4_t deq_done = __riscv_vfmul_vf_f32m4(__riscv_vfcvt_f_x_v_f32m4(input_vec, vl), scale, vl);
                    vfloat32m4_t result = __riscv_vfadd_vv_f32m4(deq_done, res_vec, vl);
                    
                    __riscv_vse32_v_f32m4(output_f32 + output_offset + k, result, vl);
                }
            }
        }
    }
...
void fuse_ops_DEQUANT_SILU(void * input,void *output,void * input_scale,void *weight_scale,int dim_i,int dim_j,uint64_t input_stride,uint64_t output_stride, void* output_scale)
{
    ...
    for (int s = 0; s < dim_i; s++) {
        float* row_input  = &input[s * dim_j];
        float* row_output = &output[s * dim_j];
        size_t avl, vl;
        vl = __riscv_vsetvl_e32m4(avl);
        float_t scale = input_scale_f32[s] * weight_scale_f32[0];
        for (int i = 0, avl = dim_j; avl > 0; i += vl, avl -= vl) {
            vl = __riscv_vsetvl_e32m4(avl);
            vint32m4_t input_vec = __riscv_vle32_v_i32m4(&row_input[i], vl);
...
void fuse_ops_DEQUANT_HADAMARD_QUANTSTAGE1(void * input,void *output,void * input_scale,void *weight_scale,int dim_i,int dim_j,uint64_t input_stride,uint64_t output_stride, void* output_max)
{
    ...
    for (int s = 0; s < dim_i; s++) {
        ...
        for (int i = 0, avl = dim_j; avl > 0; i += vl, avl -= vl) {
            ...
            vfloat32m4_t hadamard_vec = __riscv_vle32_v_f32m4(&row_output[i], vl);
            vfloat32m4_t hadamard_res = __riscv_vfmul_vv_f32m4(x, hadamard_vec, vl);

            __riscv_vse32_v_f32m4(&row_output[i], hadamard_res, vl);
...
//   void afater_operation(void * input,void *output,void * input_scale,void *weight_scale,int dim_i,int dim_j,uint64_t input_stride,uint64_t output_stride, void* output_scale);
  ...
  if(after_ops != FUSE_MASKED_SOFTMAX_KVSCALE_BF16CVRT && after_ops != FUSE_DEQUANT_RESADD_RMSNORM_QUANT && after_ops != NO_ACTIVATION)
  {
    int Tile_I = DIM_M / 64;
    int Tile_J = DIM_N / 64;
