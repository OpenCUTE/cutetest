...
static float  ffn_gate_scale[1] = {1.0f};
...
void fuse_ops_DEQUANT_SILU(void * input,void *output,void * input_scale,void *weight_scale,int dim_i,int dim_j,uint64_t input_stride,uint64_t output_stride, void* output_scale)
{
    ...
    int32_t* input_row_pipe0 = (int32_t*)(input);
    float_t* output_row_pipe0 = (float_t*)(output);
    float_t scale_pipe0 = input_scale_f32[0] * weight_scale_f32[0];
    vint32m4_t input_vec_pipe0 = __riscv_vle32_v_i32m4(input_row_pipe0, vl);

    for (int s = 1; s < dim_i; s++) {
        int input_offset = s * input_stride;
        int output_offset = s * output_stride;

        vfloat32m4_t x = __riscv_vfmul_vf_f32m4(__riscv_vfcvt_f_x_v_f32m4(input_vec_pipe0, 64), scale_pipe0, 64);
        input_row_pipe0 = (int32_t*)(input + input_offset);
        input_vec_pipe0 = __riscv_vle32_v_i32m4(input_row_pipe0, 64);
        vfloat32m4_t exp_neg_x = vec_exp(__riscv_vfneg_v_f32m4(x, 64), 64);
        vfloat32m4_t silu = __riscv_vfdiv_vv_f32m4(x, __riscv_vfadd_vf_f32m4(exp_neg_x, 1.0f, 64), 64);//div性能很差啊
        __riscv_vse32_v_f32m4(output_row_pipe0, silu, 64);
        scale_pipe0 = input_scale_f32[s] * weight_scale_f32[0];
        output_row_pipe0 = (float_t*)(output + output_offset);
    }
    vfloat32m4_t x = __riscv_vfmul_vf_f32m4(__riscv_vfcvt_f_x_v_f32m4(input_vec_pipe0, 64), scale_pipe0, 64);
    vfloat32m4_t exp_neg_x = vec_exp(__riscv_vfneg_v_f32m4(x, 64), 64);
    vfloat32m4_t silu = __riscv_vfdiv_vv_f32m4(x, __riscv_vfadd_vf_f32m4(exp_neg_x, 1.0f, 64), 64);//div性能很差啊
