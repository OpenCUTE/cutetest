...
void softmax_after_operation(acc_t  input[64][3072], int dim_i,int dim_j,elem_t * output,acc_scale_t scale,int stride_c)
{
    ...
    for (size_t i = 0; i < dim_i; i++) {
        ...
        //round 1
        acc_t max_q[16] __attribute__((aligned(64)));
        ...
        __asm__ volatile (
            ...
            : [input_slice] "r" (input_slice), [max_q] "r" (max_q), [j_itermax_m4] "r" (j_itermax_m4)  // 输入约束
            : "t0", "t1", "a3", "a4", "a5", "v0", "v1", "v2", "v3", "v4", "v5", "v6", "v7", "memory" // 破坏描述符
        );

        //round 2
        acc_t sum_exp[16] __attribute__((aligned(64)));
        // for (size_t j = 0; j < dim_j/8; j++) 
        // {
        //     for (size_t k = 0; k < 8; k++)
        //     {
        ...
        __asm__ volatile (
            ...
            : [input_slice] "r" (input_slice), [sum_exp] "r" (sum_exp), [j_itermax_m4] "r" (j_itermax_m4), [max_q_max] "r" (max_q_max), [qln2_inv] "r" (qln2_inv), [qln2] "r" (qln2), [qb] "r" (qb), [qc] "r" (qc)  // 输入约束
        ...
        //round 3
        scale_t factor_8[16] __attribute__((aligned(64)));
        // for (size_t j = 0; j < dim_j/8; j++) 
        // {
        //     for (size_t k = 0; k < 8; k++)
        //     {
        ...
        __asm__ volatile (
            ...
            : // 输出寄存器（空）
            : [input_slice] "r" (input_slice), [output_slice] "r" (output_slice), [j_itermax_store] "r" (j_itermax_store), [factor_8] "r" (factor_8), [factor] "f" (factor), [elem_t_max] "r" (elem_t_max), [elem_t_min] "r" (elem_t_min)  // 输入约束
