...
# 输入文件名
input_file = f"/home/yuanbin/chipyard/sims/verilator/output/chipyard.harness.TestHarness.CUTETestConfig128bitdram512bitL2Widen3issueBoom/transformer_cute.out"  # 要筛选的文件
output_file = "/home/yuanbin/chipyard/generators/cute/cutetest/transformer/cutetest/CML_Store_trace.out"  # 保存筛选结果的文件
...
# 读取matmul_value_mnk_113_128_128_fullbias.h文件的所有内容
with open(f"../sublayer/Q.txt", "r") as f:
    content = f.read()

    data = content.split("Q_buf = ")[1].split(";")[0]
...
with open(input_file, "r") as infile:
    ...
    for line in infile:
        ...
        if line.find("WriteRequest: RequestVirtualAddr=") != -1:
            ...
            # print(offset)
            data_golden = Q_buf[major_index][reduce_index : reduce_index + 8]
