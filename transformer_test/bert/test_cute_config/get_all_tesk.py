...
def run_command(x, progress, total, lock):
    ...
    for ddrtype in DDR_type:
        ...
        (set -o pipefail &&  ./build/simulator-chipyard.harness-CUTE4TopsShuttle512D512V512M512Sysbus512Membus1CoreConfig\
            +permissive \
            +dramsim +dramsim_ini_dir=../../dramsim_config/dramsim2_ini_{ddrtype}_per_s +max-cycles=800000000 +loadmem=../ibert-base-{x}.riscv     \
            +verbose \
            +permissive-off \
            ../ibert-base-{x}.riscv \
            \
            </dev/null  | tee ./log/ibert_{x}_{ddrtype}_task.log)
        """

        script_path = f"./test/ibert_{x}_{ddrtype}_task.sh"
