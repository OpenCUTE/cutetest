...
def run_command(x, progress, total, lock):
    ...
    (set -o pipefail &&  {root_dir}sims/verilator/simulator-chipyard.harness-CUTEM2564TCUTEShuttle512D512V512M256S1CoreConfig \
        +permissive \
        +dramsim +dramsim_ini_dir={root_dir}generators/testchipip/src/main/resources/dramsim3_ini +max-cycles=50000000 +loadmem={root_dir}generators/cute/cutetest/resnet50_512mem/vec_ops_conv_{x}.riscv     \
        +verbose \
        +permissive-off \
        {root_dir}generators/cute/cutetest/resnet50_512mem/vec_ops_conv_{x}.riscv \
        \
        </dev/null 2> >(spike-dasm > /dev/null) | tee {root_dir}sims/verilator/output/chipyard.harness.TestHarness.CUTEM2564TCUTEShuttle512D512V512M256S1CoreConfig/vec_ops_conv_{x}.log)
    """
