input_file = "/home/yuanbin/merge-test/chipyard/sims/verilator/output/chipyard.harness.TestHarness.CUTEShuttle512D512V512M256S1CoreConfig/softmax_cute.out"  # 要筛选的文件
...
with open(filter_file, "r") as infile:
    ...
    tensor_block_baseaddr = 0x80145300
    line_index = 0
    iter_n = 0
    iter_m = 0
    # 遍历文件每一行
    for line in infile:
        ...
        if (addr_trace >= tensor_block_baseaddr and addr_trace < tensor_block_baseaddr + application_m * application_n):
            ...
            data_golden = ff1_golden[iter_m + tile_M * scp_m][iter_n + tile_N * scp_n : iter_n + tile_N * scp_n + 64]

            if (data_golden != data_trace):
                print(f"addr{hex(addr_golden)},\ndata_trace {data_trace},\ndata_golden{data_golden}")
                exit(0)
