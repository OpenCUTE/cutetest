input_file = "/home/yuanbin/merge-test/chipyard/sims/verilator/output/chipyard.harness.TestHarness.CUTEShuttle512D512V512M256S1CoreConfig/ibert_3.out"  # 要筛选的文件
...
with open(filter_file, "r") as infile:
    ...
    for line in infile:
        ...
        if (addr_trace >= tensor_block_baseaddr and addr_trace < tensor_block_baseaddr + application_m * application_n):
            ...
            data_golden = result_golden[iter_m + tile_M * scp_m][iter_n + tile_N * scp_n + head * application_n // head_num : iter_n + tile_N * scp_n + head * application_n // head_num + 64]
            ...
            if (iter_n >= scp_n):
                ...
                if (iter_m >= scp_m):
                    iter_m = 0
                    tile_N += 1
                    if (tile_N * scp_n >= application_n // head_num):
                        tile_N = 0
                        tile_M += 1
                        if (tile_M * scp_m >= application_m):
                            tile_M = 0
                            head += 1
