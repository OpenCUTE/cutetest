...
void softmax_after_operation(acc_t  input[64][3072], int dim_i,int dim_j,elem_t * output,acc_scale_t scale,int stride_c)
{
    ...
    for (size_t i = 0; i < i_itermax; i++) {
        ...
        scale_t factor_16[16] __attribute__((aligned(64)));
        
        //round 1
        for (size_t j = 0; j < 16; j++) {
            acc_t *input_slice = &input[i * 16 + j][0];
            ...
            __asm__ volatile (
                ...
                : "t0", "a3", "a4", "a5", "v0", "v1", "v2", "v3", "v4", "v5", "v6", "v7", "memory" // 破坏描述符
            );
            ...
            __asm__ volatile (
                ...
                : [input_slice] "r" (input_slice), [j_itermax_m4] "r" (j_itermax_m4), [max_q_max] "r" (max_q_max), [qln2_inv] "r" (qln2_inv), [qln2] "r" (qln2), [qb] "r" (qb), [qc] "r" (qc)  // 输入约束
                : "t0", "t1", "t2", "t3", "t4", "t5", "a3", "a4", "a5", "a6", "v0", "v1", "v2", "v3", "v4", "v5", "v6", "v7", "v8", "v9", "v10", "v11", "v12", "v13", "v14", "v15", "memory" // 破坏描述符
            );
            factor_16[j] = (float) sum_exp_sum;
        }
        ...
        __asm__ volatile (
            ...
            : // 输出寄存器（空）
            : [factor_16] "r" (factor_16)  // 输入约束
            : "a6", "t0", "f1", "v0", "memory" // 破坏描述符
        );

        for (size_t j = 0; j < 16; j++) {
            scale_t factor = factor_16[j];
            //round 3
            // for (size_t j = 0; j < dim_j/8; j++) 
            // {
            //     for (size_t k = 0; k < 8; k++)
            ...
            // }
            acc_t *input_slice = &input[i * 16 + j][0];
            elem_t *output_slice = output + (i * 16 + j) * stride_c; 
