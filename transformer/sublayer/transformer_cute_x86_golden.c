...
void print_array_acc(acc_t *array, int len) {
    printf("{");
    for (int i = 0; i < len; i++)
    {
        printf("%d,",array[i]);
...
void print_array_2d_elem(elem_t *array, int len1, int len2) {
    printf("{\n");
    for (int i = 0; i < len1; i++) {
        printf("{");
        for (int j = 0; j < len2; j++) {
            printf("%d,", array[i * len2 + j]);
        }
        printf("},\n");
    }
    printf("};\n");
...
void print_array_outbuf(elem_t *array, int m, int n, int maxsize) {
    printf("{\n");
    int print_num = 0;
    for (int i = 0; i < m; i++) {
        printf("{");
        for (int j = 0; j < n; j++) {
            printf("%d,", print_num < maxsize ? array[print_num] : 0);
            print_num++;
        }
        printf("},\n");
    }
...
void print_array_3d_elem(elem_t *array, int len1, int len2, int len3) {
    printf("{\n");
    for (int i = 0; i < len1; i++) {
        printf("{\n");
        for (int j = 0; j < len2; j++) {
            ...
            for (int k = 0; k < len3; k++) {
                ...
                printf("%d,", array[i * len2 * len3 + j * len3 + k]);
            }
            printf("},\n");
        }
        printf("},\n");
...
void print_QKV_buf(elem_t *array, int len1, int len2, int len3) {
    printf("{\n");
    for (int i = 0; i < len1; i++) {
        if (i == 2) {
            printf("{\n");
            for (int j = 0; j < len3; j++) {
                ...
                for (int k = 0; k < len2; k++) {
                    // fprintf(fp, "%d,", array[i][j][k]);
                    printf("%d,", array[i * len2 * len3 + k * len3 + j]);
                }
                // printf("},\n");
            }
            printf("},\n");
...
void print_32_elem(elem_t * array) {
    printf("{");
    for (int i = 31; i >= 0; i--) {
        printf("%x, ", array[i]);
    }
...
void write_array_acc(FILE *fp, acc_t *array, int len) {
    fprintf(fp,"{");
    for (int i = 0; i < len; i++)
    {
...
void array_acc_rand(acc_t *array, int len) {
    for (int i = 0; i < len; i++)
    {
        array[i] = rand()%893465 - 446732;
    }
}

void array_elem_2d_rand(elem_t *array, int len1, int len2) {
    for (int i = 0; i < len1; i++) {
        for (int j = 0; j < len2; j++) {
            array[i * len2 + j] = rand()%256-128;
        }
    }
}

void array_elem_3d_rand(elem_t *array, int len1, int len2, int len3) {
    for (int i = 0; i < len1; i++) {
        for (int j = 0; j < len2; j++) {
            for (int k = 0; k < len3; k++) {
                // fprintf(fp, "%d,", array[i][j][k]);
                array[i * len2 * len3 + j * len3 + k] = rand()%256-128;
            }
        }
    }
}

void write_array_2d_elem(FILE *fp, elem_t *array, int len1, int len2) {
    fprintf(fp, "{\n");
    for (int i = 0; i < len1; i++) {
        fprintf(fp, "{");
        for (int j = 0; j < len2; j++) {
            fprintf(fp, "%d,", array[i * len2 + j]);
        }
        fprintf(fp, "},\n");
    }
    fprintf(fp, "};\n");
...
void write_array_2d_elem_trans(FILE *fp, elem_t *array, int len1, int len2) {
    fprintf(fp, "{\n");
    for (int i = 0; i < len2; i++) {
        fprintf(fp, "{");
        for (int j = 0; j < len1; j++) {
            fprintf(fp, "%d,", array[j * len2 + i]);
        }
        fprintf(fp, "},\n");
    }
    fprintf(fp, "};\n");
...
void write_array_3d_elem(FILE *fp, elem_t *array, int len1, int len2, int len3) {
    fprintf(fp, "{\n");
    for (int i = 0; i < len1; i++) {
        fprintf(fp, "{\n");
        for (int j = 0; j < len2; j++) {
            ...
            for (int k = 0; k < len3; k++) {
                // fprintf(fp, "%d,", array[i][j][k]);
                fprintf(fp, "%d,", array[i * len2 * len3 + j * len3 + k]);
            }
            fprintf(fp, "},\n");
        }
        fprintf(fp, "},\n");
...
void write_array_3d_elem_trans(FILE *fp, elem_t *array, int len1, int len2, int len3) {
    fprintf(fp, "{\n");
    for (int i = 0; i < len1; i++) {
        fprintf(fp, "{\n");
        for (int j = 0; j < len3; j++) {
            ...
            for (int k = 0; k < len2; k++) {
                // fprintf(fp, "%d,", array[i][j][k]);
                fprintf(fp, "%d,", array[i * len2 * len3 + k * len3 + j]);
            }
            fprintf(fp, "},\n");
        }
        fprintf(fp, "},\n");
  ...
  if (act != LAYERNORM && act != SOFTMAX && !transA && !transB && DIM_I % 4 == 0 && DIM_J % 4 == 0) {
    ...
    for (size_t i = 0; i < DIM_I; i++) {
      ...
      if (act == LAYERNORM) {
        ...
        // for (size_t j = 0; j < DIM_J / 32; j++) {
        //     printf("c[%d] = ", j * 32);
        //     print_32_elem(C + (i * stride_C) + j * 32);
        // }
        ...
        // for (size_t j = 0; j < DIM_J / 8; j++) {
        //     printf("c_buffer[%d] = ", j * 8);
        //     print_8_acc(c_buffer + j * 8);
        // }
        ...
        for (size_t j = 0; j < DIM_J; j++) {
          ...
          acc_t qp = q + z * qln2;
          acc_t q_exp = (qp + qb)*(qp + qb) + qc;
        //   printf("j:%d, q:%x, z:%x, qp:%x, q_exp:%x\n", j, q, z, qp, q_exp);
          c_buffer[j] = q_exp >> z;
          sum_exp += c_buffer[j];
        }
        ...
        elem_t * attn_buf, elem_t * out_buf)
{
    ...
    printf("QKV_buf = ");
    print_QKV_buf(Q_buf, 3, seq_len, hidden_dim);
    // exit(0);
    ...
    for (int head = 0; head < num_heads; head++) {
        ...
        tiled_matmul_auto(seq_len, seq_len, hidden_dim_per_head,
            ...
            MVIN_SCALE_IDENTITY, MVIN_SCALE_IDENTITY, MVIN_SCALE_IDENTITY,
            /*SOFTMAX*/ SOFTMAX, /*scale=*/ ACC_SCALE_IDENTITY, /*bert_scale=*/ 0.5,
            /*repeating_bias=*/ false,
    ...
    printf("attn_buf = ");
    print_array_3d_elem(attn_buf, num_heads, seq_len, seq_len);
    ...
    printf("out_buf = ");
    print_array_outbuf(out_buf, seq_len, expansion_dim, seq_len * hidden_dim);
    ...
    printf("out = ");
    print_array_2d_elem(out, seq_len, hidden_dim);
    ...
    tiled_resadd_auto(seq_len, hidden_dim,
        ...
        CPU);
    printf("resadd1_buf = ");
    print_array_2d_elem(resadd_out, seq_len, hidden_dim);
        ...
        elem_t * out_buf)
{
    ...
    printf("ff1_buf = ");
    print_array_2d_elem(out_buf, seq_len, expansion_dim);
...
#define ENCODER_DECODER(hidden_dim, expansion_dim, num_heads, cross_num_heads, seq_len, compression_factor, input, enc_out, output) ({ \
    ...
    array_elem_3d_rand(Wqkvo, 4, hidden_dim, hidden_dim); \
    static elem_t Wqkvo_cross[4][hidden_dim][hidden_dim]; \
    array_elem_3d_rand(Wqkvo_cross, 4, hidden_dim, hidden_dim); \
    static elem_t ff_w[2][hidden_dim*expansion_dim]; \
    array_elem_2d_rand(ff_w, 2, hidden_dim*expansion_dim); \
    static acc_t ff1_b[expansion_dim]; \
    array_acc_rand(ff1_b, expansion_dim); \
    static acc_t ff2_b[hidden_dim]; \
    array_acc_rand(ff2_b, hidden_dim); \
    \
    ...
    \
    fprintf(fp,"static const elem_t Wqkvo[4][%d][%d] __attribute__((aligned(256))) = ", hidden_dim, hidden_dim); \
    write_array_3d_elem_trans(fp, Wqkvo, 4, hidden_dim, hidden_dim); \
    fprintf(fp,"static const elem_t Wqkvo_cross[4][%d][%d] __attribute__((aligned(256))) = ", hidden_dim, hidden_dim); \
    write_array_3d_elem(fp, Wqkvo_cross, 4, hidden_dim, hidden_dim); \
    fprintf(fp,"static const elem_t ff_w[2][%d] __attribute__((aligned(256))) = ", hidden_dim*expansion_dim); \
    write_array_2d_elem(fp, ff_w, 2, hidden_dim*expansion_dim); \
    fprintf(fp,"static const elem_t ff1_w[%d][%d] __attribute__((aligned(256))) = ", expansion_dim, hidden_dim); \
    write_array_2d_elem_trans(fp, &ff_w[0][0], hidden_dim, expansion_dim); \
    fprintf(fp,"static const elem_t ff2_w[%d][%d] __attribute__((aligned(256))) = ", hidden_dim, expansion_dim); \
    write_array_2d_elem_trans(fp, &ff_w[1][0], expansion_dim, hidden_dim); \
    fprintf(fp,"static const acc_t ff1_b[%d] __attribute__((aligned(256))) = ", expansion_dim); \
    write_array_acc(fp, ff1_b, expansion_dim); \
    fprintf(fp,"static const acc_t ff2_b[%d] __attribute__((aligned(256))) = ", hidden_dim); \
    write_array_acc(fp, ff2_b, hidden_dim); \
...
#define PRINT_ENCODER_DECODER(name, is_encoder, hidden_dim, expansion_dim, num_heads, cross_num_heads, seq_len, compression_factor) { \
    ...
    array_elem_2d_rand(input, seq_len, hidden_dim); \
    static elem_t enc_out[seq_len][hidden_dim]; \
    array_elem_2d_rand(enc_out, seq_len, hidden_dim); \
    ...
    fprintf(fp,"%s\n","typedef int32_t acc_t;"); \
    fprintf(fp,"%s\n","typedef int64_t full_t;"); \
    fprintf(fp,"#define NAME \"%s\"\n",name); \
    fprintf(fp,"#define IS_ENCODER %d\n",is_encoder); \
    fprintf(fp,"#define HIDDEN_DIM %d\n",hidden_dim); \
    fprintf(fp,"#define EXPANSION_DIM %d\n", expansion_dim); \
    fprintf(fp,"#define NUM_HEADS %d\n", num_heads); \
    fprintf(fp,"#define CROSS_NUM_HEADS %d\n", cross_num_heads); \
    fprintf(fp,"#define SEQ_LEN %d\n", seq_len); \
    fprintf(fp,"#define COMPRESSION_FACTOR %d\n", compression_factor); \
    fprintf(fp,"static const elem_t input[%d][%d] __attribute__((aligned(256))) = ", seq_len, hidden_dim); \
    write_array_2d_elem(fp, input, seq_len, hidden_dim); \
    fprintf(fp,"static const elem_t enc_out[%d][%d] __attribute__((aligned(256))) = ", seq_len, hidden_dim); \
    write_array_2d_elem(fp, enc_out, seq_len, hidden_dim); \
    \
    \
...
int main (int argc, char * argv[]) {
    float fnum = (float) 1 / 1762;
    fnum = fnum * -198225;
    printf("round -112.5000076 to %d\n", (int) ROUND_NEAR_EVEN(fnum));
