...
void softmax_after_operation(acc_t  input[64][3072], int dim_i,int dim_j,elem_t * output,acc_scale_t scale,int stride_c)
{
    ...
    printf("qb:%d\n", qb);
    printf("qc:%d\n", qc);
    int j_itermax = dim_j / 8;
    int stride_in = 3072 * 4;

    for (size_t i = 0; i < dim_i; i++) {
        
        //round 1
        acc_t max_q[8] __attribute__((aligned(64))) = {0,0,0,0,0,0,0,0};
        ...
        __asm__ volatile (
            ...
            "mv a4, %[max_q]                    \n" // a4 = max_q 起始地址
            "mv a5, %[j_itermax]                \n"
            "li t0, -2147483648                 \n" // max_q[8] __attribute__((aligned(64))) = {-2147483648,-2147483648,-2147483648,-2147483648,-2147483648,-2147483648,-2147483648,-2147483648}
            "sw t0,   (a4)                      \n"
            "sw	t0, 4 (a4)                      \n"                           
            "sw	t0, 8 (a4)                      \n"                           
            "sw	t0, 12(a4)                      \n"                           
        ...
        //round 2
        acc_t sum_exp[8] __attribute__((aligned(64))) = {0,0,0,0,0,0,0,0};
        // for (size_t j = 0; j < dim_j/8; j++) 
        // {
        //     for (size_t k = 0; k < 8; k++)
        //     {
        ...
        //round 3
        scale_t factor_8[8] __attribute__((aligned(64))) = {factor,factor,factor,factor,factor,factor,factor,factor};
        // for (size_t j = 0; j < dim_j/8; j++) 
        // {
        //     for (size_t k = 0; k < 8; k++)
        //     {
...
void layernorm_after_operation(acc_t  input[64][3072], int dim_i,int dim_j,elem_t * output,acc_scale_t scale,int stride_c)
{
    ...
    for (size_t i = 0; i < dim_i; i++) {
        ...
        acc_t sum[8] __attribute__((aligned(64))) = {0,0,0,0,0,0,0,0};
        // for (size_t j = 0; j < dim_j/8; j++)
        // {
        //     for (size_t k = 0; k < 8; k++)
        //     {
        ...
        acc_t mean[8] __attribute__((aligned(64))) = {t_mean,t_mean,t_mean,t_mean,t_mean,t_mean,t_mean,t_mean};

        acc_t total_err_sq[8] __attribute__((aligned(64))) = {0,0,0,0,0,0,0,0};
        // for (size_t j = 0; j < dim_j/8; j++)
        // {
        //     for (size_t k = 0; k < 8; k++)
        //     {
        ...
        acc_scale_t stddev[8] __attribute__((aligned(64))) = {stddev_f,stddev_f,stddev_f,stddev_f,stddev_f,stddev_f,stddev_f,stddev_f};
...
void Gelu_after_operation(acc_t  input[64][3072], int dim_i,int dim_j,elem_t * output,acc_scale_t scale,int stride_c)
{
    ...
    for (size_t i = 0; i < dim_i; i++) {
        ...
        __asm__ volatile (
            ...
            // "vmv.v.i  v9, 1           \n" 
            "vmslt.vi v0, v8, 0      \n" 
            // "vmerge.vim v6, v6, -1, v0      \n" 
            // "vmslt.vi v0, v1, 0      \n" 
            // "vmerge.vim v7, v7, -1, v0      \n" 
            // "vmslt.vi v0, v2, 0      \n" 
...
//             acc_t q_tanh = q_tanh_input/2.333;
//             q = q * (0.5 * (1 + q_tanh));
//             elem_t* c = output + i * stride_c + j;
//             // Clip result
//             q = q > elem_t_max ? elem_t_max : (q < elem_t_min ? elem_t_min : q);
