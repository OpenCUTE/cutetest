    ...
    for task in [["8GB",tops_8GB], ["16GB",tops_16GB], ["32GB",tops_32GB], ["64GB",tops_64GB]]:
        ...
        for i in range(4):
            ...
            (set -o pipefail &&  ./build/simulator-chipyard.harness-CUTE{tops[i]}TopsSCP{SCP_size[i]}Config\
                ...
                \
                </dev/null  | tee ./log/cute_Matmul_mnk_512_512_{x*256}_zeroinit_transpose_{type}_{tops[i]}Tops.log)
            """

            script_path = f"./test/{tops[i]}Tops/{tops[i]}Tops_{type}ddr_mat_512_512_{x*256}.sh"
