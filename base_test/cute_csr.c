...
int main(void) {
    ...
    int M = APPLICATION_M & 0xFFFF;
    int N = APPLICATION_N & 0xFFFF;
    int K = APPLICATION_K & 0xFFFF;
    uint64_t cfgData1 = M | (N << 20) | (K << 40);
    printf("CUTE_MNK_KERNALSTRIDE_CONFIG_FUNCTOPS: 0x%lx\n, KERNEL_STRIDE: %d\n", cfgData1, KERNEL_STRIDE);

    element_type = element_type & 0xFF;
    bias_type = bias_type & 0xFF;
    int transpose_result = TRANSPOSE_RESULT & 0xFF;
    int conv_stride = CONV_STRIDE & 0xFF;
    int conv_oh_max = CONV_OH_MAX & 0x7FFF;
    int conv_ow_max = CONV_OW_MAX & 0x7FFF;
    int kernel_size = KERNEL_SIZE & 0xF;
    int conv_oh_per_add = CONV_OH_PER_ADD & 0x7FFF;
    int conv_ow_per_add = CONV_OW_PER_ADD & 0x7FFF;
    int conv_oh_index = CONV_OH_INDEX & 0x7FFF;
    int conv_ow_index = CONV_OW_INDEX & 0x7FFF;
